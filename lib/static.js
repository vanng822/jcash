...
StaticHandler.getCssManager = function(config) {
	if(!cssManager) {
		config = underscore.extend(globalSettings, config);
		cssManager = new CssManager(config.pathCss, config.locationCss, config.baseurlCss, config.active, config.inmemory, config.sqwishStrict);
	}
	return cssManager;
};
...
StaticHandler.getJsManager = function(config) {
	if(!jsManager) {
		config = underscore.extend(globalSettings, config);
		jsManager = new JsManager(config.pathJs, config.locationJs, config.baseurlJs, config.active, config.inmemory, config.strictSemicolons);
	}
	return jsManager;
};
 ...
 *
 */
var Manager = function(path, location, baseurl, active, inmemory, strict) {
	this.path = path;
...
Manager.prototype = {
	...
	_setAndCache : function(files, errcb) {
		...
		funk.run(function() {
			...
			md5dist = distPath + '/' + md5ofContent + '.' + manager.getExtention();
...
var CssManager = function(path, location, baseurl, active, inmemory, strict) {
	Manager.call(this, path, location, baseurl, active, inmemory, strict, 'css');
};
...
var JsManager = function(path, location, baseurl, active, inmemory, strict) {
	Manager.call(this, path, location, baseurl, active, inmemory, strict, 'js');
};

util.inherits(JsManager, Manager);

JsManager.prototype.minify = function(bundled) {
	return uglify(bundled.join(';'), {strict_semicolons: this.strict});
};
