...
Manager.prototype = {
	...
	dump : function(filename, options) {
		...
		blocks.forEach(function(block) {
			var pre = ( options.prekey ? options.prekey + '.' : '');
			var cached = self._getCache(self._getCacheKey(self.files[block]));
			content += pre + block + '.' + options.location + ' = ' + cached.location + "\n";
			content += pre + block + '.' + options.minifiedContent + ' = ' + options.escape(cached.minifiedContent) + "\n";
			content += pre + block + '.' + options.md5OfContent + ' = ' + cached.md5OfContent + "\n";
			if(self.urls.hasOwnProperty(block)) {
				self.urls[block].forEach(function(url) {
					content += pre + block + '.' + options.location + ' = ' + url + "\n";
					content += pre + block + '.' + options.minifiedContent + " = \n";
					content += pre + block + '.' + options.md5OfContent + " = \n";
				});
			}
		});
		if(options.returnValue) {
