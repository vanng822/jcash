...
var viewHelpers = StaticHandler.dynamicHelpers = {
	renderScriptTags : function(req, res) {
		return function(block) {
			...
			blocks = jsManager.getLocationMap(req.url);
	...
	renderStyleTags : function(req, res) {
		return function(block) {
			...
			blocks = cssManager.getLocationMap(req.url);
	...
	/* bottomScripts is accessible in template; provide this for reset value since it
	 * is reasonable */
 ...
 *
 * @param {Object} config
 *  configuration for creating CssManager object. It will be used only at the
 * first call
 * @return CssManager
 */
 ...
 *
 * @param {Object} config
 *  configuration for creating JsManager object. It will be used only at the
 * first call
 * @return JsManager
 */
 ...
 *
 * @param baseurl
 *  baseurl of the resource. Good when serve different domain for static
 * contents.
 *
 * @param active
 * turn on or off
...
var Manager = function(path, location, baseurl, active, inmemory, strict, dist) {
	 ...
	 *  third element: the md5 hash of the content if inmemory else null, for setting
	 * etags
	 */
	this.cache = {};
	this.cacheMap = {};
...
Manager.prototype = {
	...
	getLocationMap : function(location) {
		var blocks = [], key, i, len;

		if(this.locationMap.hasOwnProperty(StaticHandler.GLOBAL)) {
			blocks = this.locationMap[StaticHandler.GLOBAL];
		}
		if(!this._locationMapRegExps) {
			this._locationMapRegExps = [];
			keys = Object.keys(this.locationMap);
			for( i = 0, len = keys.length; i < len; i++) {
				if(keys[i] == StaticHandler.GLOBAL) {
					this._locationMapRegExps.push(null);
				} else {
					this._locationMapRegExps.push(new RegExp('^' + keys[i] + '$'));
				}
			}
		}
		for(i = 0, len = this._locationMapRegExps.length; i < len; i++) {
			if(this._locationMapRegExps[i] && this._locationMapRegExps[i].test(location)) {
				blocks = blocks.concat(this.locationMap[keys[i]]);
			}
		}
	...
	_dispatchDone : function() {
		if (this.isDone()) {
			this.emit(StaticHandler.EVENT_DONE);
		}	
	},
...
/* put after prototype of Manager; lazy define one at the time */
