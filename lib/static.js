...
StaticHandler.getCssManager = function(config) {
	if(!cssManager) {
		config = underscore.extend(globalSettings, config);
		cssManager = new Manager(config.pathCss, config.locationCss, config.baseurlCss, 'css', config.active, config.inmemory, config.sqwishStrict);
	}
	return cssManager;
};
...
StaticHandler.getJsManager = function(config) {
	if(!jsManager) {
		config = underscore.extend(globalSettings, config);
		jsManager = new Manager(config.pathJs, config.locationJs, config.baseurlJs, 'js', config.active, config.inmemory, config.sqwishStrict);
	}
	return jsManager;
};
