...
Manager.prototype = {
	...
	dump : function(filename, prekey, cacheKeys, returnValue, objectDump) {
		var blocks, self = this, prekey = prekey || '', content = '';
		cacheKeys = underscore.extend({
			location : 'location',
		...
		blocks.forEach(function(block) {
			var pre = ( prekey ? prekey + '.' : '');
			var cached = self._getCache(self._getCacheKey(self.files[block]));
			content += pre + block + '.' + cacheKeys.location + ' = ' + cached.location + "\n";
			content += pre + block + '.' + cacheKeys.minifiedContent + ' = ' + cached.minifiedContent + "\n";
			content += pre + block + '.' + cacheKeys.md5OfContent + ' = ' + cached.md5OfContent + "\n";
			if(self.urls.hasOwnProperty(block)) {
				self.urls[block].forEach(function(url) {
					content += pre + block + '.' + cacheKeys.location + ' = ' + url + "\n";
					content += pre + block + '.' + cacheKeys.minifiedContent + " = \n";
					content += pre + block + '.' + cacheKeys.md5OfContent + " = \n";
				});
			}
		});
		if(returnValue) {
