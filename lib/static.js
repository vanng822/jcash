...
StaticHandler.getCssManager = function(config) {
	if(!cssManager) {
		config = underscore.extend(globalSettings, config);
		cssManager = new Manager(config.path.css, config.location.css, config.baseurl.css, 'css', config.active, config.inmemory, config.sqwishStrict);
	}
	return cssManager;
};
...
StaticHandler.getJsManager = function(config) {
	if(!jsManager) {
		config = underscore.extend(globalSettings, config);
		jsManager = new Manager(config.path.js, config.location.js, config.baseurl.js, 'js', config.active, config.inmemory, config.sqwishStrict);
	}
	return jsManager;
};
 ...
 *
 */
var Manager = function(path, location, baseurl, type, active, inmemory, sqwishStrict) {
	this.path = path;
...
Manager.prototype = {
	...
	_setAndCache : function(files, errcb) {
		...
		funk.run(function() {
			...
			if(manager.type === 'css') {
				minified = sqwish.minify(bundled.join(), self.sqwishStrict);
			} else {
				minified = uglify(bundled.join(';'));
			}
			
			md5ofContent = md5(minified);
			md5dist = distPath + '/' + md5ofContent + '.' + manager.type;
	...
	renderTags : function(block, errcb) {
		var html = '', urls = this.getUrls(block, errcb), renderfunc, baseurl = this.baseurl;
